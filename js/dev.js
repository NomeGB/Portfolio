...
function openViewer(idx) {
    ...
    item.content.forEach(block => {
        ...
        if (block.type === 'image') {
            ...
            el.innerHTML = `
                <img class="compare-before" src="${block.before}" alt="before">
                <img class="compare-after"  src="${block.after}"  alt="after">
                <div class="compare-handle"></div>
                <span class="compare-label left">${block.labelBefore || 'Before'}</span>
                <span class="compare-label right">${block.labelAfter  || 'After'}</span>
            `;
            setTimeout(() => {
                const imgs = el.querySelectorAll('img');
                imgs.forEach(img => console.log(img.className, img.offsetWidth, img.offsetHeight, getComputedStyle(img).position));
            }, 500);
